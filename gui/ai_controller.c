    ...
    if (ctrl->rating_pending && ctrl->before_move_snapshot.valid) {
        ...
        if (strcmp(ctrl->before_move_snapshot.fen, data->fen) == 0) {
            ...
            if (should_rate) {
                ...
                else if (penalty_cp <= 45) stats.rating_label = "Good";
                else if (penalty_cp <= 100) stats.rating_label = "Inaccuracy";
                else if (penalty_cp <= 250) stats.rating_label = "Mistake";
...
AiController* ai_controller_new(GameLogic* logic, AiDialog* ai_dialog) {
    ...
    // REMOVED: ai_dialog_set_settings_changed_callback(ai_dialog, on_ai_settings_changed, controller);
    // Reason: main.c owns the callback to update the UI (RightSidePanel) immediately.
    // main.c will call ai_controller_start/stop analysis as needed via its own callback.
